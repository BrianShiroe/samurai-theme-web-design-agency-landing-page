...
export default function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Cast the response to any or a specific object to satisfy the compiler
  ...
  if (!isMounted) return null; // Safe guard for Next.js hydration

  return (
    <section
      id="contact"
      className="relative py-20 md:py-32 px-6 md:px-8 bg-main-bg overflow-hidden border-t border-brand/20 transition-colors duration-500"
    >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20">
          ...
          >
            ...
            <p className="font-mono text-[11px] md:text-sm text-main-text/50 leading-relaxed uppercase tracking-[0.15em] md:tracking-[0.2em] max-w-sm">
              Current Status: <span className="text-main-text font-bold tracking-widest">Accepting_Projects</span>{" "}
              <br />
            ...
            <div className="pt-2 md:pt-4 space-y-6">
              <motion.div whileHover={{ x: 10 }} className="group cursor-pointer inline-block">
                <p className="font-mono text-[10px] md:text-xs text-brand mb-1 md:mb-2 transition-all group-hover:tracking-widest font-bold">
                  // SECURE_COMM_LINK
                </p>
          ...
          >
            ...
            >
              <div className="grid grid-cols-1 gap-6 md:gap-8">
                ...
                <div className="relative group">
                  ...
                  <input
                    ...
                    type="text"
                    required
                    className="w-full bg-transparent border-b border-tactical-border py-2 md:py-3 font-gaming text-main-text focus:outline-none focus:border-brand transition-all placeholder:text-main-text/10 text-sm md:text-base"
                ...
                <div className="relative group">
                  ...
                  <textarea
                    ...
                    rows={2}
                    required
                    className="w-full bg-transparent border-b border-tactical-border py-2 md:py-3 font-gaming text-main-text focus:outline-none focus:border-brand transition-all placeholder:text-main-text/10 resize-none text-sm md:text-base"
              ...
              <motion.button
                ...
                type="submit"
                className="group relative w-full h-12 md:h-14 bg-transparent border border-brand/50 flex items-center justify-center overflow-hidden transition-all hover:border-brand disabled:opacity-50"
              >
                ...
                <div className="absolute top-0 right-0 p-1">
                  <div
                    className={`w-1.5 h-1.5 bg-brand group-hover:bg-main-bg transition-colors ${status === "SENDING" ? "animate-ping" : "animate-pulse"}`}
                  />
                </div>
              </motion.button>
